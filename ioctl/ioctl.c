...
// IOCTL Commands
#define IOCTL_GET_DATA _IOR('a', 1, int)
#define IOCTL_SET_DATA _IOW('a', 2, int)

static int device_open = 0;
static int ioctl_data = 0;
//ioctl_data: A placeholder to store data for IOCTL operations.
...
static long dev_ioctl(struct file *filep, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) 
	{
		//IOCTL_GET_DATA: Sends ioctl_data to user space.
		//IOCTL_SET_DATA: Updates ioctl_data from user space.
    case IOCTL_GET_DATA://Sends the current value of ioctl_data (stored in the driver) to the user space.
        if (copy_to_user((int __user *)arg, &ioctl_data, sizeof(ioctl_data))) {
            return -EFAULT;
        }
        pr_info("IOCTL_GET_DATA: Data sent to user: %d\n", ioctl_data);
        break;
    case IOCTL_SET_DATA://Updates the ioctl_data variable in the driver with a value provided by the user.
